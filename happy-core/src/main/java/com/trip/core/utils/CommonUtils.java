...
//                                GONE=1000 PERCEPTIBLE=130 SERVICE=300 ISIBLE=20
    ...
    public static boolean isNetWorkAvailable() {
        return isNetWorkAvailable(Happy.getApplicationContext());
    }
...
//                先暂时测试
    ...
    public static File getImgPathFromCache(String url) {
        FileBinaryResource resource = (FileBinaryResource)
                Fresco.getImagePipelineFactory().getMainDiskStorageCache()
                        .getResource(new SimpleCacheKey(Uri.parse(url).toString()));
